    ...
    if (method === 'POST' && action === 'create') {
        ...
        try {
            ...
            const cashfreePayload = {
                ...
                order_amount: Number(totalGross.toFixed(2)), // Ensure it's a number
                order_currency: "INR",
                customer_details: {
                ...
                // dashboard first. Sending local UUIDs will crash the Sandbox order creation.
            };
            ...
            if (!cfResponse.ok) {
                console.error("Cashfree Order Validation Failed:", cfData);
                throw new Error("Cashfree Validation Error: " + (cfData.message || JSON.stringify(cfData)));
            }

            return res.status(200).json({
                message: 'Checkout Session Initialized',
                payment_session_id: cfData.payment_session_id, // REAL SESSION ID from Cashfree
                order_id: cfData.order_id,
                orders: frontendOrdersList
            });
